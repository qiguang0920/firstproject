...
#######################################################################
#            Installs a PPTP VPN-only system for CentOS               #
# For more information please visit http://www.iewb.net               #
#######################################################################
"
...
EOF
    ...
    for Package in wget make openssl gcc-c++ ppp pptpd iptables iptables-services net-tools
    do
...
ms-dns 8.8.4.4 # 114 DNS Primary
...
ETH=`ip route | grep default | awk '{print $NF}'`
